...
CONFIG_FILE_PATH = ROC_TOOLKIT_BASE_PATH+"/build/docs/public_api/xml/"
...
def generate(generator_construct, output_dir, name_prefixes,
    ...
    git_tag = subprocess.check_output(
        ['git', 'describe', '--tags'], cwd=ROC_TOOLKIT_BASE_PATH).decode('ascii').strip()
    git_commit = subprocess.check_output(
        ['git', 'rev-parse', '--short', 'HEAD'], cwd=ROC_TOOLKIT_BASE_PATH).decode('ascii').strip()
